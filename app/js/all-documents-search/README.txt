...
- The <dl style="display: none;"> element that wraps the filter dropdown list cannot have hardcoded inline style - the JS will handle this
...
- <fieldset> shouldn't be wrapped around the search input box, a hidden input, and the submit button
...
- The dropdown of filters are shown in 2 columns, each in their own <ul> element.
    ...
    *** Let me know if you don't have strong feelings about 2 <ul> elements, because it'd be easier for me and more semantic to group them in a single <ul> element and then use flexbox (recommended), CSS columns, or another style method to wrap the results into multiple columns.
